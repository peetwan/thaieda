...
# เกณฑ์ "ตารางกว้างมาก" — เกินนี้จะสุ่มใช้ breakdown/measure เพียง 50% เพื่อความเร็ว (P3)
_WIDE_TABLE_COLS = 100
...
def _build_text(c: dict) -> tuple[str, str, str]:
    ...
    if pattern == "outlier":
        col = ev["column"]
        method = ev.get("method", "z_score")
        title = f"พบ outlier {ev['outlier_count']} แถวในคอลัมน์ '{col}' (วิธี {method})"
        desc = (
            f"คอลัมน์ '{col}' มี {ev['outlier_count']} ค่าที่เป็น outlier "
            f"({ev['percentage']:.1f}% ของข้อมูล, วิธี {method}, "
            f"ความเบ้ ≈ {ev.get('skew', 0.0):.2f}, "
...
def discover_insights(
    ...
    *,
    ...
    # P3: ตารางกว้างมาก (>100 คอลัมน์) — ใช้เพียง 50% ของ breakdown/measure ที่จัดอันดับสูงสุด
    # แทนการลองทุกคู่ (perspective = breakdown × measure จึงลดได้ ~4 เท่า) เพื่อความเร็ว
    if len(df.columns) > _WIDE_TABLE_COLS:
        keep_bd = max(1, len(breakdowns) // 2)
        keep_me = max(1, len(measures) // 2)
        if keep_bd < len(breakdowns) or keep_me < len(measures):
            notes.append(
                f"ตารางกว้าง ({len(df.columns)} คอลัมน์) — ใช้ breakdown {keep_bd}/{len(breakdowns)} "
                f"และ measure {keep_me}/{len(measures)} (50% อันดับสูงสุด) เพื่อความเร็ว"
            )
            breakdowns = breakdowns[:keep_bd]
            measures = dict(list(measures.items())[:keep_me])
...
def _detect_outlier_insights(
    ...
    for measure in measures:
        ...
        if std == 0:
            continue
        # เลือกวิธีตรวจตามการกระจาย (เบ้มาก -> MAD/IQR robust, ใกล้ปกติ -> z-score/GESD)
        # ให้สอดคล้องกับโมดูล anomaly — mean/std z-score เพียงอย่างเดียวนับ outlier ผิดมาก
        # บนข้อมูลเบ้ (mean/std ถูกบิดด้วย outlier เอง) เช่น dep_delay, fare, ราคา
        try:
        ...
        if np.isnan(skew):
            skew = 0.0
        method, mask = _choose_outlier_method(values, skew)
        if mask is None:
            continue
        ...
        if outlier_count < 5 and pct < 1.0:
            continue
        # คะแนนความสุดขั้วแบบ robust (modified z จาก MAD, fallback เป็น z ปกติ) เพื่อจัดอันดับ
        med = float(np.median(values))
        abs_dev = np.abs(values - med)
        mad = float(np.median(abs_dev))
        if mad:
            extremeness = float((0.6745 * abs_dev / mad).max()) / 7.0
        else:
            extremeness = float((abs_dev / std).max()) / 6.0
        score = min(1.0, extremeness)
        candidates.append(
            {
                "pattern": "outlier",
                "pattern_score": score,
