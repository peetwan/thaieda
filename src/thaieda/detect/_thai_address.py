...
# เลขที่: พยายามค้นหาแบบระบุชัดเจนด้วยคำนำหน้า "เลขที่" ก่อน แล้วค่อย fallback หาตัวเลขตัวแรกสุดที่อยู่ต้นข้อความสุด
_HOUSE_NUMBER_EXPLICIT_RE = re.compile(
    r"เลขที่\s*(?P<num>\d+(?:/\d+)?(?:-\d+)?)",
)
_HOUSE_NUMBER_FALLBACK_RE = re.compile(
    r"^\s*(?P<num>\d+(?:/\d+)?(?:-\d+)?)",
)
...
# สกัดชื่อตำบล/แขวงหลังคำนำหน้า (ใช้ lookahead เพื่อป้องกันการกลืนชื่อ อำเภอ/เขต/จังหวัด/รหัสไปรษณีย์)
...
# สกัดชื่ออำเภอ/เขตหลังคำนำหน้า (ใช้ lookahead เพื่อป้องกันการกลืนชื่อ จังหวัด/รหัสไปรษณีย์)
_DISTRICT_RE = re.compile(
    r"(?:อำเภอ|อ\.|เขต)(?:\s*)(?P<name>(?:(?!จังหวัด|จ\.)[ก-๛A-Za-z0-9.])+)",
)
...
def parse_thai_address(text: str) -> dict[str, str]:
    ...
    # --- เลขที่ — พยายามสกัดแบบ explicit ก่อน ค่อย fallback แบบหาตัวเลขต้นสตริง
    house_match = _HOUSE_NUMBER_EXPLICIT_RE.search(text_no_postal)
    if not house_match:
        house_match = _HOUSE_NUMBER_FALLBACK_RE.search(text_no_postal)
    if house_match:
