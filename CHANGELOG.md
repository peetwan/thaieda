...
ปรับความแม่นยำของ EDA/Clean/Insight และการนำเสนอรายงาน (A1–A6, D1–D6)
ตรวจสอบด้วย eval harness บนชุดข้อมูลจริง 11 ชุด (ไทย+อังกฤษ, 150–54k แถว). (#16)

### Fixed
- **(A1) `normalize_currency` ทำลายคอลัมน์ข้อความอิสระ** — คอลัมน์รีวิวที่เอ่ยถึงราคา
  (เช่น `150บาท`, `฿`) ถูกเข้าใจผิดว่าเป็นสกุลเงินแล้วบังคับแปลงเป็นตัวเลข → กลายเป็น NaN
  ทั้งคอลัมน์ → dedup ยุบ 40,000 แถวเหลือ 5 โดยไม่เตือน. ตอนนี้จะถือว่าเป็นสกุลเงินก็ต่อเมื่อ
  มีสัญญาณสกุลเงิน ≥10% **และ** ค่าที่เป็นเงินทั้งเซลล์ ≥50% **และ** ไม่ใช่ข้อความอิสระ
  (ความยาว/จำนวนโทเคนต่ำ); เพิ่ม guardrail ย้อนการแปลง+เตือนเมื่อ coercion ทำให้ค่าหาย >30%
  และเตือนเมื่อ dedup ลบแถว >50%.
- **(A2) วิเคราะห์ time series กับข้อมูลที่ไม่ใช่อนุกรมเวลา** — เพิ่ม `is_panel_time_axis()`
  ตรวจจับข้อมูล snapshot/panel (timestamp ซ้ำมากเทียบกับจำนวนแถว) แล้วข้ามการวิเคราะห์
  อนุกรมเวลาพร้อมหมายเหตุ ตัด insight แนวโน้ม/spike ปลอม (เช่น air4thai 175 แถว 3 timestamp).
- **(A3) คอลัมน์ ID/code/geo ถูกคิดเป็นค่าเชิงปริมาณ** — เพิ่ม `is_nonmeasure_numeric()`
  กลางที่ใช้ร่วมกัน เพื่อกัน `*_id`/`*_code`/`zip`/`lat`/`long` ออกจาก skew/kurtosis,
  correlation, outlier (สถิติ+ML), คำแนะนำ transform และ time series.
- **(A4) `fuzzy_duplicates` รายงานผลกระทบเกินจริง** — เปลี่ยนมานับเฉพาะแถวที่ไม่ใช่
  ตัวแทนหลักของแต่ละ cluster ซึ่งการ standardize จะ relabel จริง (เช่น marital_status
  46% → 0.07%).
- **(A6) Isolation Forest / LOF flag มากเกินไปเป็น "warning"** — ลดระดับเป็น `info`
  เมื่อสัดส่วนที่ flag > 5% (anomaly จริงควรเป็นของหายาก) ตัด false alarm บนหางการแจกแจง
  ตามธรรมชาติ.
- **(detect) คำว่า `long` ในชื่อคอลัมน์** — แยกเป็น full-token regex เพื่อไม่ให้
  `long_term_debt`/`long_position`/`belong` ถูกเข้าใจผิดเป็นลองจิจูดและถูกตัดออกจากการ
  วิเคราะห์ ขณะที่ `long`/`*_long` ยังถูกมองเป็นลองจิจูดตามเดิม.

### Changed
- **(A5) คะแนนคุณภาพสะท้อนปัญหาจริง** — เปลี่ยนสูตรเป็นแบบถ่วงน้ำหนักตามขนาดของปัญหา
  (`penalty = Σ severity_weight × สัดส่วนที่กระทบ`) normalize ด้วยจำนวนคอลัมน์เท่านั้น
  (เลิกหารด้วย `sqrt(จำนวนแถว)`); คอลัมน์ว่าง/ค่าหาย ≥50% ยกระดับเป็น `critical`.
  คะแนนของ dataset ที่มีปัญหาจะเปลี่ยนไป (เช่น titanic 100→79, C) — ชุดข้อมูลสะอาดยังได้ A.
  *หมายเหตุ: นี่เป็นการเปลี่ยนพฤติกรรมของค่าคะแนน หากโค้ดของคุณยึดค่าคะแนนแบบเดิม.*

### Report / Presentation
- **(D1) รวมการ์ด insight ต่อคอลัมน์** — ยุบข้อค้นพบของคอลัมน์เดียวเป็นการ์ดเดียว
  เรียง critical → warning → info พร้อม badge จำนวนข้อค้นพบ.
- **(D2) จำกัดจำนวน insight ที่แสดง** — การ์ด critical/warning แสดงเสมอ; การ์ดที่มีแต่
  info ยุบลงใน `<details>` "ดูเพิ่มเติม" (หรือตัดเหลือ 12) พร้อมบอกจำนวนที่ยุบ.
- **(D3) ป้ายเปอร์เซ็นต์ชัดเจนขึ้น** — meta line ระบุ "แถวที่กระทบ" และ "% ของแถว" ชัดเจน.
- **(D4) เว้นวรรคไทย/อังกฤษ** — เพิ่ม `_space_thai_latin()` แทรกช่องว่างที่รอยต่อ
  ไทย↔ละติน (`Pearsonสูง` → `Pearson สูง`) ในข้อความที่ render.
- **(D5) คำแนะนำสำหรับคอลัมน์ geo/ID** — เพิ่มคำแนะนำตามบริบท (geo → วิเคราะห์เชิงพื้นที่/แผนที่,
  identifier → ใช้เป็นคีย์เชื่อม) แทนคำแนะนำ log-transform/correlation ที่ไม่เหมาะกับคอลัมน์เหล่านี้.
- **(D6) สรุปการทำความสะอาดแยก cell/row** — แยก "ค่าที่เปลี่ยน" (ระดับเซลล์) ออกจาก
  "แถวที่ถูกลบ" (dedup/ลบทั้ง df) พร้อม ⚠ เตือนเมื่อแถวหาย ≥50%.

### Testing
- เพิ่มเทสต์เฉพาะของแต่ละ fix และ regenerate golden fixtures ที่ผูกกับสูตรใหม่; `pytest`
  ผ่าน 1003 รายการ (skip 2), `ruff check`/`format` สะอาด, CI ครบ 7 jobs (ubuntu+windows ×
  Python 3.10–3.13).
...
### Changed
- pip install thaieda ติดตั้งทุก dependencies โดย default ไม่ต้องใช้ [all]

## [2.1.1] - 2026-06-27

### Changed
- เกลาภาษาไทยใน README ให้อ่านลื่นขึ้น
